...
def graph_embed(X, msg_mask, N_x, N_embed, N_o, iter_level, Wnode, Wembed, W_output, b_output):
    ...
    output_nor = tf.nn.l2_normalize(output, dim=1, epsilon=1, name=None)
    
    return output, output_nor
...
class graphnn(object):
    def __init__(self,
        ...
        with tf.device(device):
            ...
            embed1, embed1_nor = graph_embed(X1, msg1_mask, N_x, N_embed, N_o, ITER_LEVEL,
                    Wnode, Wembed, W_output, b_output)  #[B, N_x]
            self.embed1 = embed1
            self.embed1_nor = embed1_nor

            X2 = tf.placeholder(Dtype, [None, None, N_x])
            msg2_mask = tf.placeholder(Dtype, [None, None, None])
            self.X2 = X2
            self.msg2_mask = msg2_mask
            embed2, embed2_nor = graph_embed(X2, msg2_mask, N_x, N_embed, N_o, ITER_LEVEL,
    ...
    def get_embed(self, X1, mask1):
        vec, vec_nor = self.sess.run(fetches=[self.embed1, self.embed1_nor],
